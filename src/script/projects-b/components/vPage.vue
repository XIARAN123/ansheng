<template>
  <div class="pageination_align">
    <div class="pageination" v-if="pageinationTotal">
      ...
      <div
        class="pagination_page"
        :class="item == pageinationCurrentPage ? 'pagination_page_active' : ''"
        v-for="(item, index) in pageinationLength"
        :key="index"
        @click="jump(item)"
      >
        {{ item }}
...
export default {
  ...
  methods: {
    jump(item) {
      ...
      this.pagers()
      this.$emit('change', this.pageinationCurrentPage)
    }, //跳转页码
    pagers() {
      //可分页数
